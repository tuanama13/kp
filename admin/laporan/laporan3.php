<?php
    ...
    // $tahun = "2018";
    // $bulan = "01";
    $sql = mysqli_query($conn, "SELECT * FROM transaksi WHERE YEAR(tgl_transaksi) ='$tahun' AND MONTH(tgl_transaksi) ='$bulan' AND grand_total <> 0 ");
...
// Page header
        ...
        function Header()
        {
            ...
            $this->Cell(80);
            $this->Cell(30,5,'Laporan Penjualan',0,1,'C');
...
// Page footer
    ...
    while ($row = mysqli_fetch_assoc($sql)) {
        ...
        $pdf->Cell(80 ,7,'Tanggal : '.$row['tgl_transaksi'],0,1);
        ...
        $sql1 = mysqli_query($conn,"SELECT * FROM detail_transaksi INNER JOIN list_brg USING(id_brg) WHERE id_transaksi = '$row[id_transaksi]'");
        while ($rows = mysqli_fetch_assoc($sql1)) {
            
            $pdf->Cell(100 ,8,$rows['nama_brg'],1,0);
            $pdf->Cell(16 ,8,$rows['jumlah_brg'],1,0);
